...
## Integridad de la semilla histórica

`assets/js/data/sports-data.js` contiene 106 partidos tomados de la versión funcional proporcionada por el usuario. `loadMatches()` normaliza la semilla con `normalizedMatch()` y después incorpora solo registros manuales que no pertenecen a ella. Así, un almacenamiento vacío, antiguo o parcialmente incompatible no elimina la bitácora histórica.

La identificación de partidos que definen campeonatos depende exclusivamente de `match.titleDecision`; no existe una lista paralela de IDs.
